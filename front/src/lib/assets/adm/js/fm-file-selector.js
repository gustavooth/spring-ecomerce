...
function addFile(fileName) {
    ...
    clone.addEventListener("click", function(e) {
        if (e.target.classList.contains("adm-fm-li")) {
            ...
            e.target.classList.remove("adm-fm-li");
            e.target.classList.add("adm-fm-li-s");
            selectedFilesPath.push(currentDir + fileName);

        }else {
            e.target.classList.remove("adm-fm-li-s");
            e.target.classList.add("adm-fm-li");
            let index = selectedFilesPath.indexOf(currentDir + fileName);
            if (index !== -1) {
                selectedFilesPath.splice(index, 1);
            }
        }
    });
    ...
    let index = selectedFilesPath.indexOf(currentDir + fileName);
    if (index !== -1) {
        clone.classList.remove("adm-fm-li");
        clone.classList.add("adm-fm-li-s");
    }
...
async function selectFiles(ext) {
    extensions = ext;

    showWindow();

    return new Promise((resolve, reject) => {
        // Verificar continuamente se a variável esperada é verdadeira
        const intervalo = setInterval(() => {
            if (close) {
                clearInterval(intervalo);
                resolve(selectedFilesPath);
                closeWindow();
            }
        }, 100);
    });
